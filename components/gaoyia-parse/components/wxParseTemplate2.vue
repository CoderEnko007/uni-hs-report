<template>
	...
	<block v-if="node.node == 'element'">
		...
		<view v-else :class="node.classStr" :style="node.styleStr">
			<block v-for="(node, index) of node.nodes" :key="index">
				<wx-parse-template :node="node" :tdWidth="tdWidth"/>
			</block>
		</view>
	</block>
...
<script>
    ...
    mounted() {
      if (this.node.hasOwnProperty('nodes') && this.node.tag=='tr') {
        let td_count = this.node.nodes.length
        this.tdWidth = this.node.tag=='tr'?690/td_count:undefined
      }
    },
	};
</script>